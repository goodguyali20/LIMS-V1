...
const RequisitionForm = ({ order, user, settings }) => {
  const now = new Date();
  return (
    <Wrapper className="print-only-premium">
      <Watermark>REQUISITION FORM</Watermark>
      <Header>
        <Logo>
      ...
      <Section>
        ...
        <InfoGrid>
          ...
          <InfoItem><strong>Priority:</strong> {order?.priority || 'NORMAL'}</InfoItem>
        </InfoGrid>
      </Section>
      <Section>
          ...
          <tbody>
            {order?.tests?.map((test, idx) => (
              <tr key={idx}>
                <td>{test}</td>
                <td>General</td>
                <td style={{color: '#10b981', fontWeight: 700}}>REQUESTED</td>
                <td>—</td>
              </tr>
            ))}
          </tbody>
