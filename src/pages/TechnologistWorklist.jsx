...
import { FaUserMd, FaListAlt, FaExclamationTriangle } from 'react-icons/fa';
...
const TechnologistWorklist = () => {
  ...
  return (
    <div className="fade-in">
        ...
        <WorklistGrid>
          {worklist.map(order => (
            <OrderCard key={order.id} priority={order.priority}>
              ...
              <ActionButton onClick={() => handleOpenModal(order)}><FaUserMd /> {t('enterResults')}</ActionButton>
            </OrderCard>
          ))}
        </WorklistGrid>
      ) : (
      ...
      {isModalOpen && selectedOrder && <ResultEntry order={selectedOrder} onClose={() => setIsModalOpen(false)} onResultsSubmitted={() => {}} />}
    </div>
  );
};
